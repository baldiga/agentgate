...
export default async function SdkTokensPage() {
  ...
  let agents: Agent[] = []
  try {
    agents = await apiFetch<Agent[]>('/api/agents', { headers })
  } catch {
    // ignore
  }
  ...
  const agentTokens = await Promise.all(
    agents.map(async a => {
      try {
        const tokens = await apiFetch<SdkToken[]>(`/api/agents/${a.id}/sdk-tokens`, { headers })
        return { agent: a, tokens }
      } catch {
        return { agent: a, tokens: [] as SdkToken[] }
      }
    })
  )
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {agentTokens.length === 0 && (
        <p className="text-sm text-muted">No agents yet. Create an agent first to generate SDK tokens.</p>
      )}
      {agentTokens.map(({ agent, tokens }) => (
        <div key={agent.id} className="flex flex-col gap-3">
          <h2 className="text-base font-semibold text-text-secondary">{agent.icon ?? '🤖'} {agent.name}</h2>
